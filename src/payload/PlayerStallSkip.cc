...
Define_Module(PlayerStallSkip)

PlayerStallSkip::PlayerStallSkip() {
...
void PlayerStallSkip::initialize(int stage)
{
    if (stage == 0)
    {
...
void PlayerStallSkip::activate(void)
{
...
void PlayerStallSkip::finish()
{

}

void PlayerStallSkip::handleMessage(cMessage *msg)
{
...
SEQUENCE_NUMBER_T PlayerStallSkip::findNextPlayablePosition(){
...
void PlayerStallSkip::handleTimerMessage(cMessage *msg)
{
...
//                m_stat->reportChunkSeek(m_id_nextChunk);
        ...
        else // expected chunk is NOT in buffer
        {
            ...
            {
                // m_chunks_hit, m_chunks_stalled, m_chunks_skipped, m_chunks_missed;
                // pointer went out of buffer ... this is bad :/
                int offset;
                if (m_id_nextChunk < m_videoBuffer->getBufferStartSeqNum()){
                    // scan the first 100 chunks if they exist
                    for (offset = m_videoBuffer->getBufferStartSeqNum()+25; offset < m_videoBuffer->getBufferStartSeqNum()+100; offset++)
                        if (m_videoBuffer->inBuffer(offset))
                            break;
                    m_chunks_skipped += (offset - m_id_nextChunk);
                    // listening support ->
                        std::vector<PlayerListener*>::iterator it;
                        for(it = mListeners.begin(); it != mListeners.end(); it++){
                            (*it)->onChunksSkipped(m_id_nextChunk, offset);
                        }
                    // <- listening support
                    m_id_nextChunk = offset;//m_videoBuffer->getBufferStartSeqNum() + 50; //TODO: better selection here also count the number of skipped chunks
                }else{
                    m_CurrentlyStalledChunks++;
                    if (m_CurrentlyStalledChunks >= param_MaxStalledChunks){
                        int nextPosition = findNextPlayablePosition();

                        if ((nextPosition >= 0) && (nextPosition-m_id_nextChunk<=m_CurrentlyStalledChunks*2)){// limit search range
                            if (nextPosition > m_id_nextChunk)
                                m_chunks_missed += (nextPosition-m_id_nextChunk);
                            m_id_nextChunk = nextPosition;
                        }else{
                            m_id_nextChunk++;
                            m_chunks_missed++;
...
void PlayerStallSkip::startPlayer()
{
...
SEQUENCE_NUMBER_T PlayerStallSkip::getCurrentPlaybackPoint(void)
{
    return m_id_nextChunk;
}

SEQUENCE_NUMBER_T PlayerStallSkip::getPrefferedNextChunk(){
...
bool PlayerStallSkip::playerStarted(void)
{
    //return m_playerStarted;
    return (m_state == PLAYER_STATE_PLAYING);
}

bool PlayerStallSkip::shouldResumePlaying(SEQUENCE_NUMBER_T seq_num)
{
...
void PlayerStallSkip::addListener(PlayerListener* listener){
    mListeners.push_back(listener);
}
void PlayerStallSkip::removeListener(PlayerListener* listener){
    std::vector<PlayerListener*>::iterator it;
    for(it = mListeners.begin(); it != mListeners.end(); it++){
        if ( (*it) == listener )
            mListeners.erase(it);
